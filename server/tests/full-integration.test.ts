...
describe("full persistent cross-component contract",()=>{
  it("runs a 30-player game with stable layouts, queue switching, ownership, UTC, and restart",async()=>{
    ...
    try { const hostA=await service.register("integration-a","a sufficiently strong password"),hostB=await service.register("integration-b","a sufficiently strong password");const a=service.authenticate(hostA.accessToken),b=service.authenticate(hostB.accessToken);const original=set("22222222-2222-4222-8222-222222222222","Original"), alternative=set("33333333-3333-4333-8333-333333333333","Alternative");
      ...
      const snapshots=views.map(v=>JSON.stringify((v.game.question as any).choices));for(let i=0;i<players.length;i++){const q=views[i].game.question as any;const answer=q.choices.find((x:any)=>x.text.startsWith("Correct"))!;service.answer(players[i].reconnectToken,q.id,answer.id);}const state=service.hostState(a,game.id);expect(state.players).toHaveLength(30);expect(state.players.every(p=>p.score===0)).toBe(true);expect(db.prepare("SELECT receipt_order,received_at FROM player_answers WHERE game_id=? ORDER BY receipt_order").all(game.id)).toHaveLength(30);const answerTimes=db.prepare("SELECT received_at FROM player_answers WHERE game_id=?").all(game.id) as {received_at:string}[];expect(answerTimes.every(x=>/Z$/.test(x.received_at))).toBe(true);
      for(let i=0;i<5;i++)expect(JSON.stringify((service.playerReconnect(players[i].reconnectToken).game.question as any).choices)).toBe(snapshots[i]);service.close(a,game.id);expect(service.hostState(a,game.id).players.filter(p=>p.score===15)).toHaveLength(1);const added=service.addSet(a,game.id,alternative,"inOrder");service.selectSet(a,game.id,added.gameSetId);expect(service.hostState(a,game.id).players).toHaveLength(30);const originalSet=db.prepare("SELECT id FROM game_question_sets WHERE game_id=? AND source_set_id=?").get(game.id,original.id) as {id:string};service.selectSet(a,game.id,originalSet.id);expect(()=>service.preview(a,game.id)).not.toThrow();service.end(a,game.id);
