...
public sealed class QuestionSetLibrary
{
    ...
    public QuestionSet Create(string title) { var set=new QuestionSet { Title=title }; RequireValidMetadata(set); var name=$"{set.Id}{QuestionSetFormat.Extension}"; QuestionSetSerializer.Save(Path.Combine(directory,name),set); Upsert(set,name); return set; }
    ...
    public void Delete(Guid id) { var e=config.QuestionSets.Single(x=>x.Id==id); var path=Path.Combine(directory,e.FileName); if(File.Exists(path)) File.Delete(path); config.QuestionSets.Remove(e); config.Save(); }
