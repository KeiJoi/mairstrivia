...
`POST /games` requires venue name, game name, a valid question set, ordering mode (`inOrder` or `shuffleOnce`), optional scoring, optional `questionTimeLimitSeconds` (an integer from `0` to `20`; `0` is manual close), and optional `cumulativeScoring`. It returns the internal ID, join code, player URL, and host state. A positive time limit automatically closes an open question at the server-authoritative deadline. When cumulative scoring is enabled, host state also includes combined player totals for that host's cumulative games; individual-game scores remain independent. Hosts use `POST /games/:gameId/question-sets`, `/question-sets/:setId/select`, `/questions/preview`, `/questions/skip`, `/questions/open`, `/questions/close`, and `/end` for lifecycle commands.
