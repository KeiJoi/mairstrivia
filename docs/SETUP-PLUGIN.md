...
7. Open the final **Settings** tab. Enter the public HTTPS backend URL, the server access password configured for the server, a username, and a user password. For a first-time host, select **Create Host Account**; usernames are 3-64 characters and passwords require at least 10 characters. For an existing account, select **Connect / Login**. The plugin stores the backend URL and username, but does not save either password.
