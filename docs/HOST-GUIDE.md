...
Open `/mairstrivia`, connect to your backend, and log in. In Question Sets, choose the set you want to use. In Game, enter the required **Venue Name** and **Game Name**, choose scoring, select the set, choose a **Question time limit** from 0–20 seconds, and create the game. `0` means no limit and the host closes the question manually; a positive limit closes it automatically and shows players a countdown. Enable **Cumulative scoring across games** when several games should also contribute to one host-wide leaderboard; each game still has its own independent winner. Share the displayed player URL or join code.
