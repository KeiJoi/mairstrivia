...
public sealed class QuestionSetTests
{
    ...
    [Fact] public void Schema_version_2_allows_three_through_nine_incorrect_answers(){var set=Valid();set.Questions[0].IncorrectAnswers=set.Questions[0].IncorrectAnswers.Take(3).ToList();Assert.Empty(QuestionSetValidator.Validate(set));set.Questions[0].IncorrectAnswers.Add("Wrong 4");Assert.Empty(QuestionSetValidator.Validate(set));}
    [Fact] public void Validator_rejects_too_few_answers_and_duplicates(){var set=Valid();set.Questions[0].IncorrectAnswers=set.Questions[0].IncorrectAnswers.Take(2).ToList();set.Questions[0].IncorrectAnswers[0]=set.Questions[0].CorrectAnswer;var issues=QuestionSetValidator.Validate(set);Assert.Contains(issues,x=>x.Message.Contains("Between 3 and 9"));Assert.Contains(issues,x=>x.Message.Contains("distinct"));}
    [Fact] public void Legacy_schema_version_1_requires_nine_answers(){var set=Valid();set.SchemaVersion=QuestionSetFormat.LegacySchemaVersion;set.Questions[0].IncorrectAnswers=set.Questions[0].IncorrectAnswers.Take(3).ToList();Assert.Contains(QuestionSetValidator.Validate(set),x=>x.Message.Contains("Exactly 9"));}
    [Fact] public void Validator_rejects_unsupported_versions_and_duplicate_ids(){var set=Valid();set.SchemaVersion=3;set.Questions.Add(set.Questions[0].Copy());set.Questions[1].Id=set.Questions[0].Id;var issues=QuestionSetValidator.Validate(set);Assert.Contains(issues,x=>x.Path=="schemaVersion");Assert.Contains(issues,x=>x.Message.Contains("unique"));}
